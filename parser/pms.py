...
CONDITION_END_COL = 13     # N 列

FORMULA_START_COL = 14     # O 列
FORMULA_END_COL = 28       # AC 列

PARAM_COL = 33             # AH 列（仅用于 Input / Output 定义区）
...
def extract_input_output_properties(df):
    ...
    for i in range(len(df)):
        ...
        # ---- Input ----
        if "Mandatory Parameter" in row_text:
            j = i + 1
            while j < len(df):
        ...
        # ---- Output ----
        if "Option & Config Parameter" in row_text:
            j = i + 1
            while j < len(df):
                name = cell_str(df.iloc[j, PARAM_COL])
                if not name:
...
def extract_formula_rules(df):
    start_idx = None
    for i in range(len(df)):
        if cell_str(df.iloc[i, 0]) == "Linkup & Default Value":
            start_idx = i
            break
...
def run_with_gui():
    root = tk.Tk()
    root.title("PMS Formula Dependency Parser")
    ...
    def run_process():
        if not input_paths:
            messagebox.showwarning(
                "Missing input",
                "Please select at least one PMS file."
            )
            return
        ...
        msg = f"Processed {len(success)} PMS file(s) successfully."
        if failed:
            msg += "\n\nFailed files:\n" + "\n".join(failed)

        messagebox.showinfo("Done", msg)

    tk.Label(root, text="Input PMS file(s):").place(x=20, y=20)
